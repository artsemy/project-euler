...
val numbersWordsMap: Map[Int, Int] = Map(1 -> "one".length, 2 -> "two".length, 3 -> "three".length,
...
def numberLetterCount(n: Int): Int =

  def countThousandthPart(n: Int, sum: Int): Int =
    val t = n / 1000
    countLetters(n % 1000, sum + numbersWordsMap(t) + numbersWordsMap(1000))
  ...
  def countHundredthPart(n: Int, sum: Int): Int =
    ...
    val and =
      if n % 100 == 0 then
        ...
        numbersWordsMap(-1)
    countLetters(n % 100, sum + numbersWordsMap(h) + numbersWordsMap(100) + and)

  def countTenthPart(n: Int, sum: Int): Int =
    if n < 20 then
      numbersWordsMap(n) + sum
    else
      val t = n / 10
      countLetters(n % 10, sum + numbersWordsMap(t * 10))
