...
def lychrelNumbersCount(upperBorder: Int): Int =
  (1 until upperBorder).count(isLychrelCandidate)

def isLychrelCandidate(n: Int): Boolean =

  @tailrec
  def loop(n: BigInt, iterCount: Int): Boolean =
    !isPalindrome(n.toString()) && (iterCount > 50 || loop(n + BigInt(n.toString.reverse), iterCount + 1))

  loop(BigInt(n) + BigInt(n.toString.reverse), 1)
